...
typedef struct HT_Info {
	int fd;
	int depth;            	//HT TOTAL DEPTH
	int rec_per_block;    	//RECORDS PER BLOCK = (BLOCK SIZE - one int to count records in block)/RECORD SIZE
	int HT_block_capacity;	//CAPACITY OF HT BLOCKS = BLOCK SIZE/INT SIZE
						  	//HT BLOCKS HOLD BLOCK IDS OF THE BLOCKS ACTING AS THE HASH TABLE BUCKETS
	int* HT;				// Hash Table
	//for statistics
	int block_count;		// Total record count
	int min_rec_in_bucket;	// Minimum records in a bucket
	int max_rec_in_bucket;	// Maximum records in a bucket
	int rec_count;			// Total record count
	int bucket_count;		// Bucket count (data blocks)
